...
from quantum_information.encoding.qram import encode_data, qram_initialize

def state_mba_distance(
    train,
    test,
    tipo="biclase",
    labels=None,
    ...
    """
    deprecated: usa `mba_distance` o modelos en `quantum_information.metrics.models`.

    Ejecuta un circuito QRAM completo y retorna el estado final del sistema.
    """

    warnings.warn(
        "state_mba_distance está deprecada; usa `mba_distance` o los modelos en "
        "`quantum_information.metrics.models`.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    if labels is not None:
        match tipo:
            case "multiclase":
                n_total += int(np.ceil(np.log2(len(np.unique(labels)))))
            case "multilabel":
