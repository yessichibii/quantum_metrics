...
from quantum_information.circuits.circuits import build_biclase_qnode, make_device
from quantum_information.preprocessing.data import to_one_hot
from quantum_information.utils.params import cross_entropy, init_params


class QMLBiClase(QuantumBaseModel):
    """
    Quantum Machine Learning Binary Classifier using parameterized quantum circuits.
    
    This classifier implements a quantum machine learning approach for binary
    ...
    >>> model_with_weights = QMLBiClase(codigo="gray", epochs=20, weights=initial_weights)
    >>> model_with_weights.fit(X, y)
    
    >>> # Using classifier without weight optimization
    >>> model_no_weights = QMLBiClase(codigo="gray", use_weights=False)
    ...
    def fit_validation(self, X: np.ndarray, y: np.ndarray, 
                      partitions: Optional[list] = None) -> 'QMLBiClase':
        """
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, X_val: Optional[np.ndarray] = None, 
           y_val: Optional[np.ndarray] = None) -> 'QMLBiClase':
        """
        ...
        if self.n_classes_ != 2:
            raise ValueError(
                f"QMLBiClase is designed for binary classification, "
        ...
        # Debug information
        if self.verbose:
            print(f"Circuit parameters: m={m}, qubits_dato={qubits_dato}, qubits_qram={qubits_qram}, n_totales={n_totales}")
        ...
        try:
            # print(X)
            circuit = build_biclase_qnode(
                X, y, n_totales, qubits_qram, qubits_dato, dev, 
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        # Debug information
        if self.verbose:
            print(f"Prediction circuit parameters: m={m}, qubits_dato={qubits_dato}, qubits_qram={qubits_qram}, n_totales={n_totales}")
        ...
        try:
            circuit = build_biclase_qnode(
                train_data, train_labels, n_totales, qubits_qram, qubits_dato, dev, 
                codigo=self.codigo, noise=self.noise, result=self.result
            )
        except Exception as e:
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        # Debug information
        if self.verbose:
            print(f"Probability prediction circuit parameters: m={m}, qubits_dato={qubits_dato}, qubits_qram={qubits_qram}, n_totales={n_totales}")
